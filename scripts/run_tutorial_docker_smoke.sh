...
docker compose -f "${root}/examples/tutorials/job_application_assistant/compose.yaml" exec -T application-coordinator ./application_client --coordinator-url http://application-coordinator:8080 --resume-resource resume://candidate/alex --job-file samples/job_description.txt | grep -F "Application draft"
docker compose -f "${root}/examples/tutorials/customer_support_copilot/compose.yaml" exec -T support-coordinator ./support_client --coordinator-url http://support-coordinator:8180 --ticket-resource ticket://northstar/billing-currency | grep -F "billing"
