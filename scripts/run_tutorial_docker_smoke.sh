...
docker compose -f "${root}/examples/tutorials/job_application_assistant/compose.yaml" exec -T application-coordinator /opt/tutorial/application_client --coordinator-url http://application-coordinator:8080 --resume-file samples/resume.txt --job-file samples/job_description.txt | grep -F "Application draft"
docker compose -f "${root}/examples/tutorials/customer_support_copilot/compose.yaml" exec -T support-coordinator /opt/tutorial/support_client --coordinator-url http://support-coordinator:8180 --ticket-file samples/billing_currency_ticket.txt | grep -F "billing"
