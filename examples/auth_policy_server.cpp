...
int main() {
  ...
  a2a::server::RestServerTransport server(
      &dispatcher, a2a::examples::BuildRestAgentCard("Auth Policy Example Agent", "http://agent.local/a2a"),
      {.rest_api_base_path = "/a2a"});

  const auto unauthorized = server.Handle({.method = "GET", .target = "/a2a/tasks/task-1", .headers = {}, .body = {}});
  if (!unauthorized.ok()) {
  ...
  const auto authorized = server.Handle(
      {.method = "GET", .target = "/a2a/tasks/task-1", .headers = {{"authorization", "Bearer token"}}, .body = {}});
  if (!authorized.ok()) {
