...
Client-side streaming subscription and event consumption.
...
```bash
cmake -S examples/fetch_content_consumer -B build-example -DA2A_EXAMPLE_APP=streaming_client
...
cmake -S examples/installed_package_consumer -B build-installed-example \
  ...
  -DA2A_EXAMPLE_APP=streaming_client
