...
int main() {
  ...
  a2a::server::GrpcServerTransport server(&dispatcher);

  (void)server;
  a2a::server::RequestContext context;
  auto response = executor.SendMessage(BuildMessageRequest(), context);
  if (!response.ok()) {
    std::cerr << "grpc_server failed: " << response.error().message() << '\n';
    return 1;
  }
  std::cout << "grpc_server service ready: yes" << '\n';
  std::cout << "grpc_server task id: " << response.value().task().id() << '\n';
  std::cout << "grpc_server state: " << response.value().task().status().state() << '\n';
  return 0;
}
