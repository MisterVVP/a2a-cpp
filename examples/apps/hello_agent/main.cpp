...
int main() {
  ...
  auto agent_card = a2a::core::AgentCardBuilder::RestPreset("Hello Agent", "http://agent.local/a2a").Build();
  ...
  if (!sent.ok()) {
    std::cerr << "hello_agent failed: " << sent.error().message() << '\n';
    return 1;
  }
  std::cout << "hello_agent task id: " << sent.value().task().id() << '\n';
  std::cout << "hello_agent final state: " << sent.value().task().status().state() << '\n';
  return 0;
}
