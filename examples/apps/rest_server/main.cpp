...
int main() {
  ...
  auto agent_card = a2a::core::AgentCardBuilder::RestPreset("REST Server Agent", "http://agent.local/a2a").Build();
  a2a::server::RestServerTransport server(&dispatcher, std::move(agent_card), {.rest_api_base_path = "/a2a"});

  const auto card = server.Handle(
      {.method = "GET", .target = "/.well-known/agent-card.json", .headers = {}, .body = {}, .remote_address = {}});
  if (!card.ok()) {
    std::cerr << "rest_server agent card failed: " << card.error().message() << '\n';
    return 1;
  }
  ...
  if (!sent.ok()) {
    std::cerr << "rest_server send failed: " << sent.error().message() << '\n';
    return 1;
  }
  std::cout << "rest_server agent-card status: " << card.value().status_code << '\n';
  std::cout << "rest_server task id: " << sent.value().task().id() << '\n';
  return 0;
}
