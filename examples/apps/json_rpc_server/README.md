...
Server-side JSON-RPC transport setup.
...
```bash
cmake -S examples/fetch_content_consumer -B build-example -DA2A_EXAMPLE_APP=json_rpc_server
...
cmake -S examples/installed_package_consumer -B build-installed-example \
  ...
  -DA2A_EXAMPLE_APP=json_rpc_server
