...
Server-side streaming publication and observation.
...
```bash
cmake -S examples/fetch_content_consumer -B build-example -DA2A_EXAMPLE_APP=streaming_server
...
cmake -S examples/installed_package_consumer -B build-installed-example \
  ...
  -DA2A_EXAMPLE_APP=streaming_server
