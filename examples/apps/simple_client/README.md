...
Client-side request construction and task fetch flow.
...
```bash
cmake -S examples/fetch_content_consumer -B build-example -DA2A_EXAMPLE_APP=simple_client
...
cmake -S examples/installed_package_consumer -B build-installed-example \
  ...
  -DA2A_EXAMPLE_APP=simple_client
