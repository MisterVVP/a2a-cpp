...
int main() {
  ...
  auto agent_card = a2a::core::AgentCardBuilder::RestPreset("Auth Policy Agent", "http://agent.local/a2a").Build();
  a2a::server::RestServerTransport server(&dispatcher, std::move(agent_card), {.rest_api_base_path = "/a2a"});

  const auto unauthenticated = server.Handle(
      {.method = "POST", .target = "/a2a/message:send", .headers = {}, .body = "{}", .remote_address = "127.0.0.1"});
  a2a::server::HttpServerRequest authenticated_request{.method = "POST",
                                                       .target = "/a2a/message:send",
                                                       .headers = {{"authorization", "Bearer example-token"}},
                                                       .body = "{}",
                                                       .remote_address = "127.0.0.1"};
  const auto authenticated = server.Handle(authenticated_request);
  if (!unauthenticated.ok() || !authenticated.ok()) {
    std::cerr << "auth_policy_server transport failed\n";
    return 1;
  }
  std::cout << "auth_policy_server unauthenticated status: " << unauthenticated.value().status_code << '\n';
  std::cout << "auth_policy_server authenticated metadata: authorization\n";
  std::cout << "auth_policy_server authenticated status: " << authenticated.value().status_code << '\n';
  return 0;
}
