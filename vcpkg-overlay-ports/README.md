...
The overlay pins the upstream source to the stable `v0.4.0` release tag. For an official `microsoft/vcpkg` registry PR, copy the port files into `ports/a2a-cpp` in a `microsoft/vcpkg` fork and replace `vcpkg_from_git` with a release archive source:
