...
std::string ErrorBody(std::string_view message, std::string_view code) {
  ...
  message_value.set_string_value(std::string(message));
  ...
  code_value.set_string_value(std::string(code));
...
core::Result<void> ParseQueryString(std::string_view raw,
  ...
  while (start <= raw.size()) {
    ...
    if (!part.empty()) {
      ...
      const auto value_raw = split == std::string_view::npos ? std::string_view{}
                                                              : part.substr(split + 1);
...
core::Result<HttpServerResponse> RestServerTransport::Handle(const HttpServerRequest& request) const {
  ...
  if (!version.ok()) {
    ...
    response.status_code = kHttpUpgradeRequired;
    response.headers["Content-Type"] = "application/json";
    response.headers[std::string(core::Version::kHeaderName)] = core::Version::HeaderValue();
    response.body = ErrorBody(version.error().message(), "unsupported_version");
    return response;
  }
  ...
  if (!rest_request.ok()) {
    HttpServerResponse response;
    response.status_code = kHttpNotFound;
    response.headers["Content-Type"] = "application/json";
    response.headers[std::string(core::Version::kHeaderName)] = core::Version::HeaderValue();
    response.body = ErrorBody("No matching route or request was malformed", "validation_error");
    return response;
  }
...
HttpServerResponse RestServerTransport::ToHttpResponse(const RestResponse& response) const {
