...
constexpr auto kValidatePostgresPushSchemaSql = std::to_array(
    ...
    "AND ((pg_catalog.length(task_delete_lock_function.prosrc) - "
    "pg_catalog.length(pg_catalog.replace(task_delete_lock_function.prosrc, $13::text, ''))) = "
    "$24::integer * pg_catalog.length($13::text) OR "
    "($1::text = pg_catalog.lower($1::text) AND "
    "pg_catalog.length(task_delete_lock_function.prosrc) - "
    "pg_catalog.length(pg_catalog.replace(task_delete_lock_function.prosrc, $14::text, '')) = "
    "$24::integer * pg_catalog.length($14::text))), "
