...
The current real wire-level scenario set covers core lifecycle operations, finite streaming, first-event task subscription, and push notification config CRUD for HTTP+JSON, JSON-RPC, and gRPC: `ListTasks_NoPagination`,
...
`GetTask_MissingTaskError`, `SendStreamingMessage_FiniteStream`,
`SubscribeToTask_FirstEventLatency`, `PushConfig_Create`, `PushConfig_Get`,
`PushConfig_List`, and `PushConfig_Delete`. The wire driver reuses one client/transport per
...
`wire_grpc`. Wire coverage includes bounded list with and without pagination, send/create, get existing, cancel working, follow-up send, missing-task get errors, finite streaming, first-event subscription, and push config create/get/list/delete. Known limitation: multi-subscriber subscription, disconnect isolation, terminal-completion subscription, and local HTTP callback fan-out are currently in-process SDK measurements rather than full wire rows.
